...
class ImportDescriptorsTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        # Byze enforces bech32m at node startup.
        self.extra_args = [["-addresstype=bech32m"],
                           ["-addresstype=bech32m", "-keypool=5"]
                          ]
    ...
    def run_test(self):
        ...
        # Byze policy: some builds enforce bech32m witness v1 only.
        # This upstream test exercises legacy + v0 segwit address variants heavily.
        try:
            w0.getnewaddress("", "legacy")
        except JSONRPCException as e:
            msg = (e.error.get("message") or "").lower()
            if e.error.get("code") in (-5, -8) and (
                "only bech32m witness v1 addresses are supported" in msg
                or ("requires" in msg and "bech32m" in msg)
            ):
                raise SkipTest("Byze enforces bech32m-only addresses; skipping legacy/v0 descriptor import variants test")
            raise
        ...
        self.test_importdesc({"desc": descsum_create("sh(wpkh(" + key.pubkey + "))"),
                              ...
                              success=False,
                              error_code=-8,
                              error_message="Active descriptors must be ranged; Byze supports only bech32m (witness v1) for active use")
