...
# Tests that are known to be resource-heavy in Byze (e.g. RandomX/quantum)
# and can cause OS-level OOM kills when multiple node clusters initialize at once.
# Keep this list minimal and prefer harness scheduling over changing test logic.
SERIAL_SCRIPTS = {
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Cache directory used for pre-mined chain data across parallel jobs.
    #
    # Byze builds are often produced via scripts that may run under sudo,
    # leaving the default build cache directory non-writable for the user. In
    # that case, transparently fall back to a tmpdir-based cache so cookie auth
    # and datadir creation work reliably.
    default_cache_dir = "%s/test/cache" % build_dir
    cache_dir = default_cache_dir
    try:
        os.makedirs(cache_dir, exist_ok=True)
        if not os.access(cache_dir, os.W_OK):
            raise PermissionError(f"Cache dir not writable: {cache_dir}")
    except OSError:
...
class TestHandler:
    ...
    @staticmethod
    def _is_serial_test(test: str) -> bool:
        # `test` may include args: "foo.py --bar". Only the script name matters.
        return test.split()[0] in SERIAL_SCRIPTS
    ...
    def get_next(self):
        while len(self.jobs) < self.num_jobs and self.test_list:
            # Ensure SERIAL_SCRIPTS never overlap with any other test.
            if any(self._is_serial_test(t) for t in self.jobs.values()):
                break
            if self._is_serial_test(self.test_list[0]) and self.jobs:
                break
            # Add tests
