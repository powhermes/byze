    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, versions=None):
        ...
        for i in range(num_nodes):
            ...
            init = dict(
                chain=self.chain,
                rpchost=rpchost,
                    # Cache creation startup can be heavier than normal (RandomX init).
                    # Keep a larger lower bound for RPC bring-up.
                    timewait=max(self.rpc_timeout, 240),
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    cache_node_dir,
                    chain=self.chain,
                    # Byze: be explicit about bind+port. Some builds reject bare `bind=IP`
                    # during early init which prevents RPC cookie creation.
                    extra_conf=[f"bind=127.0.0.1:{p2p_port(CACHE_NODE_ID)}"],
                    # Byze: cache generation must initialize the same quantum paths as
                    # the rest of the functional suite (RandomX + block sig enforcement).
                    extra_args=["-enforcequantumblocksigs=1", "-fallbackfee=0.001", "-logratelimit=0"],
                    rpchost=None,
                    # Cache creation startup can be heavier than normal (RandomX init).
                    # Keep a larger lower bound for RPC bring-up.
                    timewait=max(self.rpc_timeout, 240),
