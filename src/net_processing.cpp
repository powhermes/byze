...
void PeerManagerImpl::TransactionAddedToMempool(const NewMempoolTransactionInfo& tx, uint64_t mempool_sequence)
{
    const CTransactionRef& ptx{tx.info.m_tx};
    RelayTransaction(ptx->GetHash(), ptx->GetWitnessHash());
    // mempool_sequence is unused; present for CValidationInterface compatibility.
    (void)mempool_sequence;
}
...
bool PeerManagerImpl::CheckHeadersPoW(const std::vector<CBlockHeader>& headers, const Consensus::Params& consensusParams, Peer& peer)
{
    ...
    if (!HasValidProofOfWork(headers, consensusParams)) {
        // Identify the first header that fails PoW so debugging can match it to the peer's fork.
        for (const CBlockHeader& h : headers) {
            if (CheckProofOfWork(h, h.nBits, consensusParams)) continue;

            const uint256 hdr_hash{h.GetHash()};
            const uint256 pow_hash{RandomXHash(h)};
            const auto target{DeriveTarget(h.nBits, consensusParams.powLimit)};

            if (target) {
                LogPrintf("[pow-debug] rejecting headers: header_pow_invalid hash=%s prev=%s time=%u bits=%08x nonce=%u pow_hash=%s target=%s pow_gt_target=%d\n",
                          hdr_hash.ToString(),
                          h.hashPrevBlock.ToString(),
                          h.nTime,
                          h.nBits,
                          h.nNonce,
                          pow_hash.ToString(),
                          target->GetHex(),
                          UintToArith256(pow_hash) > *target);
            } else {
                LogPrintf("[pow-debug] rejecting headers: header_pow_invalid (target-derive-failed) hash=%s prev=%s time=%u bits=%08x nonce=%u pow_hash=%s pow_limit=%s\n",
                          hdr_hash.ToString(),
                          h.hashPrevBlock.ToString(),
                          h.nTime,
                          h.nBits,
                                            ...
                                            bool via_compact_block)
{
    ...
    {
        const CBlockHeader& first{headers.front()};
        LogDebug(BCLog::NET,
                 "[pow-debug] received headers (pre-pow-check): first=%s prev=%s time=%u bits=%08x nonce=%u count=%u via_compact=%d peer=%d\n",
                 first.GetHash().ToString(),
                 first.hashPrevBlock.ToString(),
                 first.nTime,
                 first.nBits,
                 first.nNonce,
                 nCount,
...
void PeerManagerImpl::ProcessValidTx(NodeId nodeid, const CTransactionRef& tx, const std::list<CTransactionRef>& replaced_transactions)
{
    ...
    // Relay is driven by ValidationSignals::TransactionAddedToMempool (PeerManagerImpl) for all
    // mempool admissions, including reorg resurrection and local RPC submission.
...
void PeerManagerImpl::ProcessCompactBlockTxns(CNode& pfrom, Peer& peer, const BlockTransactions& block_transactions)
{
    ...
    bool fBlockRead{false};
    {
        ...
        if (status == READ_STATUS_INVALID) {
            ...
            // BIP152 compact blocks carry only the header + short txids; Byze quantum tail is not included.
            ...
            if (IsQuantumBlockSignaturePolicyRequired(*pblock, Params().GetConsensus(), /*fCheckPOW=*/true) &&
                ...
                if (first_in_flight) {
                    ...
                    RemoveBlockRequest(block_transactions.blockhash, pfrom.GetId());
                    LogDebug(BCLog::NET, "Peer %d sent compact block for %s without Byze quantum tail; waiting on first in-flight download\n",
                        pfrom.GetId(), block_transactions.blockhash.ToString());
                }
                return;
            }
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        // Multimap ensures ordering of outstanding requests. It's either empty or first in line.
        first_in_flight = already_in_flight == 0 || (range_flight.first->second.first == pfrom.GetId());
    ...
    if (msg_type == NetMsgType::HEADERS)
    {
        ...
        headers.reserve(nCount);
        // GETHEADERS responses are built from `std::vector<CBlock>` (see GETHEADERS handler) so the
        // payload matches `CBlock` serialization (vtx + Byze quantum signature blob), not the legacy
        // `CBlockHeader` + trailing varint tx-count encoding.
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        if (CanServeBlocks(*peer) && ((sync_blocks_and_headers_from_peer && (!IsLimitedPeer(*peer) || allow_limited_peer)) || allow_limited_peer) && state.vBlocksInFlight.size() < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
