                                      ...
                                      bool wait_callback)
{
    ...
    // Fresh admissions relay via PeerManagerImpl::TransactionAddedToMempool. Re-announce when the
    // tx was already in the mempool (no new ATMP, so no TransactionAddedToMempool).
    if (broadcast_method == TxBroadcast::MEMPOOL_AND_BROADCAST_TO_ALL && !submitted_fresh) {
