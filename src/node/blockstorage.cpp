    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                // Skip PoW check for genesis block (it's pre-validated in chainparams)
                // For Byze, we need to use RandomX-based PoW check, so construct a block header
                // Check if this is the genesis block by height (more reliable than hash comparison)
                const bool is_genesis = (pindexNew->nHeight == 0);
                if (!is_genesis) {
                    // Construct block header from CBlockIndex for RandomX PoW validation
                    CBlockHeader header;
                    ...
                    if (!CheckProofOfWork(header, pindexNew->nBits, consensusParams)) {
...
bool BlockManager::ReadBlock(CBlock& block, const FlatFilePos& pos, const std::optional<uint256>& expected_hash) const
{
    ...
    // Check the header (using RandomX for Byze)
    // Special case for genesis block: verify hash matches expected genesis hash
    // (genesis block is pre-validated in chainparams, so we trust the hash match)
    const bool is_genesis = (block_hash == GetConsensus().hashGenesisBlock);
    if (is_genesis) {
        // For genesis block, just verify the hash matches (proof-of-work was validated during mining)
        if (expected_hash && block_hash != *expected_hash) {
            LogError("Genesis block hash mismatch at %s (%s != %s)", 
                     pos.ToString(), block_hash.ToString(), expected_hash->ToString());
            return false;
        }
    } else {
        // For all other blocks, check proof-of-work
