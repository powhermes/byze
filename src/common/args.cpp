...
const char * const BITCOIN_CONF_FILENAME = "byze.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Byze
    //   new: C:\Users\Username\AppData\Local\Byze
    // macOS: ~/Library/Application Support/Byze
    // Unix-like: ~/.byze
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Byze";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Byze";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Byze";
#else
    // Unix-like
    return pathRet / ".byze";
#endif
