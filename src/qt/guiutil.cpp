...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Byze address (e.g. %1)").arg(
...
{
    // Return if URI is not valid or has an unexpected scheme.
    // For production compatibility we accept legacy bitcoin: links, but Byze uses byze:.
    if (!uri.isValid()) return false;
    const QString scheme{uri.scheme()};
    if (scheme != QString("byze") && scheme != QString("bitcoin")) {
        return false;
    }
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("byze:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
