...
/**
 * Port numbers for incoming Tor connections and RPC.
 * Byze uses ports 8882 (mainnet RPC), 18882 (testnet RPC), 18843 (regtest RPC).
 */
std::unique_ptr<CBaseChainParams> CreateBaseChainParams(const ChainType chain)
{
    switch (chain) {
    case ChainType::MAIN:
        return std::make_unique<CBaseChainParams>("", 8882);  // Byze mainnet RPC port
    ...
    case ChainType::REGTEST:
        return std::make_unique<CBaseChainParams>("regtest", 18843);  // Keep same for regtest
    }
    assert(false);
}
