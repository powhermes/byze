...
OutputType CWallet::TransactionChangeType(const std::optional<OutputType>& change_type, const std::vector<CRecipient>& vecSend) const
{
    ...
    if (change_type) {
        if (*change_type != OutputType::BECH32M) {
            throw std::runtime_error("Byze requires bech32m change outputs");
        }
        return *change_type;
    }
    (void)vecSend;
    ...
    throw std::runtime_error("Byze requires internal BECH32M ScriptPubKeyMan for change");
}
...
util::Result<CTxDestination> CWallet::GetNewDestination(const OutputType type, const std::string label)
{
    if (type != OutputType::BECH32M) {
        return util::Error{_("Byze requires bech32m quantum addresses only")};
    }
...
{
    if (type != OutputType::BECH32M) {
        return util::Error{_("Byze requires bech32m quantum change addresses only")};
    }
...
std::shared_ptr<CWallet> CWallet::Create(WalletContext& context, const std::string& name, std::unique_ptr<WalletDatabase> database, uint64_t wallet_creation_flags, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    walletInstance->m_keypool_size = std::max(args.GetIntArg("-keypool", DEFAULT_QUANTUM_KEYPOOL_SIZE), int64_t{1});
    ...
    if (!args.GetArg("-addresstype", "").empty()) {
        ...
        if (parsed.value() != OutputType::BECH32M) {
            error = _("Byze requires -addresstype=bech32m");
            return nullptr;
        }
    ...
    if (!args.GetArg("-changetype", "").empty()) {
        ...
        if (parsed.value() != OutputType::BECH32M) {
            error = _("Byze requires -changetype=bech32m");
            return nullptr;
        }
...
void CWallet::LoadActiveScriptPubKeyMan(uint256 id, OutputType type, bool internal)
{
    ...
    Assert(IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS));
    if (type != OutputType::BECH32M) {
        throw std::runtime_error("Byze descriptor wallets only support bech32m ScriptPubKeyManagers");
    }
