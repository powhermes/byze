...
static void EnforceQuantumOutputType(OutputType output_type)
{
    if (output_type != OutputType::BECH32M) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Byze requires bech32m quantum addresses only");
    }
}
...
RPCHelpMan getnewaddress()
{
    return RPCHelpMan{
        "getnewaddress",
        "Returns a new Byze address for receiving payments.\n"
                "If 'label' is specified, it is added to the address book \n"
                "so payments received with the address will be associated with 'label'.\n",
                {
                    {"label", RPCArg::Type::STR, RPCArg::Default{""}, "The label name for the address to be linked to. It can also be set to the empty string \"\" to represent the default label. The label does not need to exist, it will be created if there is no label by the given name."},
                    {"address_type", RPCArg::Type::STR, RPCArg::DefaultHint{"bech32m"}, "The address type to use. Byze supports only \"bech32m\"."},
                },
                RPCResult{
...
RPCHelpMan getrawchangeaddress()
{
    return RPCHelpMan{
        "getrawchangeaddress",
        "Returns a new Byze address, for receiving change.\n"
