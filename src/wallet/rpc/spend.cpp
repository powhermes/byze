...
namespace wallet {
static bool IsQuantumScriptPubKey(const CScript& script)
{
    int witness_version{-1};
    std::vector<unsigned char> witness_program;
    if (!script.IsWitnessProgram(witness_version, witness_program)) return false;
    if (witness_version != 1 || witness_program.size() != WITNESS_V1_TAPROOT_SIZE) return false;
    return script.size() == 2 + WITNESS_V1_TAPROOT_SIZE && script[0] == OP_1 && script[1] == WITNESS_V1_TAPROOT_SIZE;
}

static bool IsQuantumDestination(const CTxDestination& destination)
{
    return IsQuantumScriptPubKey(GetScriptForDestination(destination));
}

static void EnsureQuantumRecipients(const std::vector<CRecipient>& recipients)
{
    for (const auto& recipient : recipients) {
        if (!IsQuantumDestination(recipient.dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Byze wallet sends support only bech32m witness v1 destinations");
        }
    }
}
...
UniValue SendMoney(CWallet& wallet, const CCoinControl &coin_control, std::vector<CRecipient> &recipients, mapValue_t map_value, bool verbose)
{
    ...
    EnsureQuantumRecipients(recipients);
    ...
    if (!options.isNull()) {
        if (options.type() == UniValue::VBOOL) {
            ...
            if (options.exists("changeAddress") || options.exists("change_address")) {
                ...
                if (!IsValidDestination(dest) || !IsQuantumDestination(dest)) {
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Change address must be a valid bech32m witness v1 address");
                }
            ...
            if (options.exists("change_type")) {
                ...
                if (std::optional<OutputType> parsed = ParseOutputType(options["change_type"].get_str())) {
                    if (*parsed != OutputType::BECH32M) {
                        throw JSONRPCError(RPC_INVALID_PARAMETER, "Byze requires change_type=bech32m");
                    }
                ...
                "entire package have the given fee rate, not the resulting transaction.\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "",
                        Cat<std::vector<RPCArg>>(
                        {
                            ...
                            {"change_type", RPCArg::Type::STR, RPCArg::DefaultHint{"bech32m"}, "The output type to use. Only valid if changeAddress is not specified. Byze supports only \"bech32m\"."},
        ...
        "\nSend a transaction.\n",
        {
            ...
            {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "",
                Cat<std::vector<RPCArg>>(
                {
                    ...
                    {"change_type", RPCArg::Type::STR, RPCArg::DefaultHint{"bech32m"}, "The output type to use. Only valid if change_address is not specified. Byze supports only \"bech32m\"."},
