...
static UniValue ProcessDescriptorImport(CWallet& wallet, const UniValue& data, const int64_t timestamp) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    try {
        ...
        if (active && !parsed_descs.at(0)->IsRange()) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Active descriptors must be ranged; Byze supports only bech32m (witness v1) for active use");
        }
        ...
        for (size_t j = 0; j < parsed_descs.size(); ++j) {
            ...
            if (active) {
                ...
                } else if (*w_desc.descriptor->GetOutputType() != OutputType::BECH32M) {
                    throw JSONRPCError(RPC_INVALID_PARAMETER, "Byze supports only bech32m descriptors for active use");
                } else {
