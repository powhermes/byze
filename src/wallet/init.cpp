...
{
    argsman.AddArg("-addresstype", strprintf("What type of addresses to use (Byze supports only \"%s\")", FormatOutputType(OutputType::BECH32M)), ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
    ...
    argsman.AddArg("-changetype",
                   strprintf("What type of change to use (Byze supports only \"%s\")", FormatOutputType(OutputType::BECH32M)),
    ...
    argsman.AddArg("-keypool=<n>", strprintf("Set key pool size to <n> (default: %u). Byze quantum keys are expensive to generate, so the default is small; raise it for a larger pre-derived gap-limit at the cost of slower wallet creation. Warning: Smaller sizes may increase the risk of losing funds when restoring from an old backup, if none of the addresses in the original keypool have been used.", DEFAULT_QUANTUM_KEYPOOL_SIZE), ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
