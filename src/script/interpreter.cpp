...
bool GenericTransactionSignatureChecker<T>::VerifyQuantumDualSignature(std::span<const unsigned char> xmss_sig, std::span<const unsigned char> sphincs_sig, std::span<const unsigned char> dual_pubkey, const uint256& sighash) const
{
    ...
    std::vector<uint8_t> xmss_pubkey_data(dual_pubkey.begin(), dual_pubkey.begin() + XMSS_PUBKEY_SIZE);
    std::vector<uint8_t> sphincs_pubkey_data(dual_pubkey.begin() + XMSS_PUBKEY_SIZE, dual_pubkey.end());
    std::vector<uint8_t> xmss_sig_data(xmss_sig.begin(), xmss_sig.end());
    std::vector<uint8_t> sphincs_sig_data(sphincs_sig.begin(), sphincs_sig.end());
    ...
    if (!xmss_pubkey.load(xmss_pubkey_data) ||
        ...
        !xmss_signature.load(xmss_sig_data) ||
        !sphincs_signature.load(sphincs_sig_data)) {
        return false;
    }

    return xmss_pubkey.verify(sighash, xmss_signature) &&
           sphincs_pubkey.verify(sighash, sphincs_signature);
}
...
template <class T>
bool GenericTransactionSignatureChecker<T>::CheckQuantumDualSignature(std::span<const unsigned char> xmss_sig, std::span<const unsigned char> sphincs_sig, std::span<const unsigned char> dual_pubkey, SigVersion sigversion, ScriptExecutionData& execdata, ScriptError* serror) const
{
    ...
    uint256 sighash;
    if (!SignatureHashSchnorr(sighash, execdata, *txTo, nIn, SIGHASH_DEFAULT, SigVersion::TAPROOT, *this->txdata, m_mdb)) {
        return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
    }
    if (!VerifyQuantumDualSignature(xmss_sig, sphincs_sig, dual_pubkey, sighash)) {
        return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
    }
    return true;
}
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, script_verify_flags flags, const BaseSignatureChecker& checker, ScriptError* serror, bool is_p2sh)
{
    ...
    if (witversion == 0) {
        ...
        // Byze quantum-safe witness v1 spend:
        // witness stack = [ xmss_signature, sphincs_signature, dual_public_key_bundle ]
        // scriptPubKey program = SHA256(dual_public_key_bundle)
        if (stack.size() != 3) return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);

        execdata.m_annex_present = false;
        execdata.m_annex_init = true;

        const valtype& xmss_sig = stack[0];
        const valtype& sphincs_sig = stack[1];
        const valtype& dual_pubkey = stack[2];

        unsigned char dual_pubkey_hash[32];
        CSHA256().Write(dual_pubkey.data(), dual_pubkey.size()).Finalize(dual_pubkey_hash);
        if (std::memcmp(dual_pubkey_hash, program.data(), program.size()) != 0) {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        }

        if (!checker.CheckQuantumDualSignature(xmss_sig, sphincs_sig, dual_pubkey, SigVersion::TAPROOT, execdata, serror)) {
            return false;
        }
        return set_success(serror);
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CScriptWitness* witness, script_verify_flags flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    // Byze requires witness v1 quantum-safe spends only.
...
{
    if (witversion == 1 && witprogram.size() == WITNESS_V1_TAPROOT_SIZE) {
        // Quantum witness validation performs 2 mandatory signature checks.
        return 2;
    }
