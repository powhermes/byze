...
public:
    ...
    virtual bool CreateQuantumSig(const SigningProvider& provider, std::vector<unsigned char>& xmss_sig, std::vector<unsigned char>& sphincs_sig, std::vector<unsigned char>& dual_pubkey_bundle, const WitnessV1Taproot& output_key, SigVersion sigversion) const =0;
