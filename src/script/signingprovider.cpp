...
bool HidingSigningProvider::SignQuantumSighash(const uint256& sighash, std::span<const unsigned char> output_program, std::vector<unsigned char>& xmss_sig, std::vector<unsigned char>& sphincs_sig, std::vector<unsigned char>& dual_pubkey_bundle) const
{
    return m_provider->SignQuantumSighash(sighash, output_program, xmss_sig, sphincs_sig, dual_pubkey_bundle);
}
...
bool MultiSigningProvider::SignQuantumSighash(const uint256& sighash, std::span<const unsigned char> output_program, std::vector<unsigned char>& xmss_sig, std::vector<unsigned char>& sphincs_sig, std::vector<unsigned char>& dual_pubkey_bundle) const
{
    for (const auto& provider: m_providers) {
        if (provider->SignQuantumSighash(sighash, output_program, xmss_sig, sphincs_sig, dual_pubkey_bundle)) return true;
    }
    return false;
}
