...
bool MutableTransactionSignatureCreator::CreateQuantumSig(const SigningProvider& provider, std::vector<unsigned char>& xmss_sig, std::vector<unsigned char>& sphincs_sig, std::vector<unsigned char>& dual_pubkey_bundle, const WitnessV1Taproot& output_key, SigVersion sigversion) const
{
    if (sigversion != SigVersion::TAPROOT) return false;
    std::optional<uint256> sighash = ComputeSchnorrSignatureHash(/*leaf_hash=*/nullptr, SigVersion::TAPROOT);
    if (!sighash.has_value()) return false;
    return provider.SignQuantumSighash(*sighash, std::span<const unsigned char>(output_key.begin(), output_key.size()), xmss_sig, sphincs_sig, dual_pubkey_bundle);
}
...
[[maybe_unused]] static bool SignTaproot(const SigningProvider& provider, const BaseSignatureCreator& creator, const WitnessV1Taproot& output, SignatureData& sigdata, std::vector<valtype>& result)
{
                     ...
                     std::vector<valtype>& ret, TxoutType& whichTypeRet, SigVersion sigversion, SignatureData& sigdata)
{
    ...
    {
        std::vector<unsigned char> xmss_sig;
        std::vector<unsigned char> sphincs_sig;
        std::vector<unsigned char> dual_pubkey_bundle;
        const WitnessV1Taproot output_key{XOnlyPubKey{vSolutions[0]}};
        if (!creator.CreateQuantumSig(provider, xmss_sig, sphincs_sig, dual_pubkey_bundle, output_key, SigVersion::TAPROOT)) {
            return false;
        }
        ret.push_back(std::move(xmss_sig));
        ret.push_back(std::move(sphincs_sig));
        ret.push_back(std::move(dual_pubkey_bundle));
        return true;
    }
