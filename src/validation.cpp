...
void Chainstate::InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    if (tip) {
        ...
        LogWarning("%s: chain tip is null (chain may be empty during initialization)\n", __func__);
    }
}
...
script_verify_flags GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    // Byze consensus: never disable P2SH/WITNESS/TAPROOT verification bits.
    // This avoids legacy exception tables accidentally bypassing quantum witness validation.
    ...
    DisconnectedBlockTransactions& disconnectpool)
{
    ...
    // Remove conflicting transactions from the mempool.;
    if (m_mempool) {
        LogPrintf("[quantum-mempool] ConnectTip calling removeForBlock height=%d vtx=%u mempool_before=%u\n",
                  pindexNew->nHeight, (unsigned)block_to_connect->vtx.size(), (unsigned)m_mempool->size());
        m_mempool->removeForBlock(block_to_connect->vtx, pindexNew->nHeight);
        LogPrintf("[quantum-mempool] ConnectTip removeForBlock done height=%d mempool_after=%u\n",
                  pindexNew->nHeight, (unsigned)m_mempool->size());
...
{
    // Special case for genesis block: skip proof-of-work check since it's pre-validated in chainparams
    const bool is_genesis = (block.GetHash() == consensusParams.hashGenesisBlock);
    
    // Check proof of work matches claimed amount (using RandomX for Byze)
    // Skip for genesis block as it's already validated in chainparams
    if (fCheckPOW && !is_genesis && !CheckProofOfWork(block, block.nBits, consensusParams))
...
bool IsQuantumBlockSignaturePolicyRequired(const CBlock& block, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    if (!require_quantum) return false;
    const bool isGenesis = (block.GetHash() == consensusParams.hashGenesisBlock);
    if (isGenesis) return false;
    const bool isTemplate = (block.nNonce == 0 && !fCheckPOW);
...
static bool VerifyBlockQuantumSignatures(const CBlock& block)
{
    ...
    LogDebug(BCLog::VALIDATION, "[quantum-cmp] path=verify stage=entry block_hash=%s xmss_prefix=%s sphincs_prefix=%s bundle_prefix=%s\n",
        block_hash_str,
    ...
    std::vector<uint8_t> xmss_pubkey_data(
        block.quantum_signatures.dual_public_key.begin(),
    ...
    crypto::xmss_signature xmss_signature;
    crypto::sphincs_signature sphincs_signature;
    if (!xmss_pubkey.load(xmss_pubkey_data) ||
        !sphincs_pubkey.load(sphincs_pubkey_data) ||
        !xmss_signature.load(block.quantum_signatures.xmss_signature) ||
        !sphincs_signature.load(block.quantum_signatures.sphincs_signature)) {
        LogError("[byze] block_sig_reject hash=%s reason=crypto_deserialize_failed\n", block_hash_str);
        LogDebug(BCLog::VALIDATION, "[quantum-cmp] path=verify stage=deserialize_failed block_hash=%s\n", block_hash_str);
        return false;
    }

    // Same message as mining: CBlockHeader::GetHash() / RandomX over header fields only (see primitives/block.cpp).
    if (!xmss_pubkey.verify(block_hash, xmss_signature)) {
        LogError("[byze] block_sig_reject hash=%s reason=xmss_verify_failed\n", block_hash_str);
        LogDebug(BCLog::VALIDATION, "[quantum-cmp] path=verify stage=crypto_verify block_hash=%s xmss_ok=0\n", block_hash_str);
        return false;
    }
    if (!sphincs_pubkey.verify(block_hash, sphincs_signature)) {
        LogError("[byze] block_sig_reject hash=%s reason=sphincs_verify_failed\n", block_hash_str);
        LogDebug(BCLog::VALIDATION, "[quantum-cmp] path=verify stage=crypto_verify block_hash=%s sphincs_ok=0\n", block_hash_str);
        return false;
    }
    return true;
}
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (IsQuantumBlockSignaturePolicyRequired(block, consensusParams, fCheckPOW)) {
        if (block.quantum_signatures.xmss_signature.empty() || block.quantum_signatures.sphincs_signature.empty()) {
            LogError("[byze] block_sig_reject hash=%s reason=missing_tail xmss_sz=%zu sphincs_sz=%zu dual_sz=%zu\n",
                block.GetHash().ToString(),
                block.quantum_signatures.xmss_signature.size(),
                block.quantum_signatures.sphincs_signature.size(),
                block.quantum_signatures.dual_public_key.size());
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-quantum-sig-missing", "Quantum-safe signatures required (both XMSS and SPHINCS+ required)");
        }
        if (!VerifyBlockQuantumSignatures(block)) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-quantum-sig", "Invalid quantum-safe block signatures");
        }
    } else {
        // On testnet/signet: Validate quantum signatures if present, but don't require them
        if (!block.quantum_signatures.xmss_signature.empty() || !block.quantum_signatures.sphincs_signature.empty()) {
            // If any quantum signature is present, all must be present and valid
            if (block.quantum_signatures.xmss_signature.empty() || block.quantum_signatures.sphincs_signature.empty()) {
                LogError("[byze] block_sig_reject hash=%s reason=partial_tail xmss_sz=%zu sphincs_sz=%zu dual_sz=%zu\n",
                    block.GetHash().ToString(),
                    block.quantum_signatures.xmss_signature.size(),
                    block.quantum_signatures.sphincs_signature.size(),
                    block.quantum_signatures.dual_public_key.size());
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-quantum-sig", "Partial quantum-safe signatures (both XMSS and SPHINCS+ required)");
            }
            if (!VerifyBlockQuantumSignatures(block)) {
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-quantum-sig", "Invalid quantum-safe block signatures");
            }
        }
        // If quantum signatures are empty on testnet/signet, allow it (Bitcoin-compatible)
    }
