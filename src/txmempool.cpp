...
void CTxMemPool::removeForBlock(const std::vector<CTransactionRef>& vtx, unsigned int nBlockHeight)
{
    ...
    if (mapTx.size() || mapNextTx.size() || mapDeltas.size()) {
        ...
        for (const auto& tx : vtx) {
            if (tx->HasWitness()) {
                LogPrintf("[quantum-mempool] removeForBlock height=%u blocktx txid=%s wtxid=%s vin=%u vout=%u wit0_elems=%u\n",
                          nBlockHeight,
                          tx->GetHash().ToString(),
                          tx->GetWitnessHash().ToString(),
                          tx->vin.size(),
                          tx->vout.size(),
                          (tx->vin.size() > 0 ? tx->vin[0].scriptWitness.stack.size() : 0));
            }
            txiter it = mapTx.find(tx->GetHash());
            if (it != mapTx.end()) {
                if (tx->HasWitness()) {
                    LogPrintf("[quantum-mempool] removeForBlock found mempooltx txid=%s wtxid=%s removing=1\n",
                ...
                LogPrintf("[quantum-mempool] removeForBlock not_found_in_mempool txid=%s wtxid=%s mempool_size=%u\n",
                          tx->GetHash().ToString(),
                          tx->GetWitnessHash().ToString(),
                          (unsigned)mapTx.size());
            }
