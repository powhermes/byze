...
public:
    CMainParams() {
        ...
        // Byze mainnet: no assumeutxo data. The values previously listed here were
        // copied from Bitcoin mainnet (blocks 840k/880k/910k) and can never exist
        // on the Byze chain; leaving them in would let a user supply a Bitcoin
        // snapshot whose base block hash cannot match any Byze block.
        m_assumeutxo_data = {};

        // No reliable Byze tx stats yet; zeroed rather than carrying Bitcoin's
        // getchaintxstats output, which inflated sync progress estimates.
...
public:
    CTestNet4Params() {
        ...
        // Byze testnet4 has no Bitcoin-derived minimum work or assumevalid block;
        // the values previously here were Bitcoin testnet4 parameters.
        consensus.nMinimumChainWork = uint256{};
        consensus.defaultAssumeValid = uint256{};
        ...
        // Byze testnet4: no assumeutxo data. The entry previously listed here was
        // a Bitcoin testnet4 snapshot base that cannot exist on this chain.
        m_assumeutxo_data = {};
