#!/usr/bin/env bash
# Package Byze v0.2.3 macOS arm64 release (CLI + GUI + DMG)
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
BUILD="${ROOT}/build"
OUT="${ROOT}/release/v0.2.3/macos-arm64"
DMG="${ROOT}/release/byze-core-0.2.3-macos-arm64.dmg"
...
echo "==> Place binary into app bundle"
mkdir -p "${BUILD}/Bitcoin-Qt.app/Contents/MacOS"
cp "${BUILD}/bin/byze-qt" "${BUILD}/Bitcoin-Qt.app/Contents/MacOS/Byze-Qt"
...
"${ROOT}/release/v0.2.3/resign-macos-app.sh" "${BUILD}/dist/Bitcoin-Qt.app" "-"
...
hdiutil create -volname "Byze Core v0.2.3 arm64" -srcfolder "${STAGE}" -ov -format UDZO "${DMG}"

echo "==> Checksums"
(
  cd "${ROOT}/release"
  shasum -a 256 "$(basename "${DMG}")" > SHA256SUMS_Mac_arm64.txt
  cd v0.2.3/macos-arm64
...
echo "  ${ROOT}/release/v0.2.3/sign-macos-release.sh"
