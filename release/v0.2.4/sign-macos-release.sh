...
"${ROOT}/release/v${VER}/resign-macos-app.sh" "${APP}" "${IDENTITY}"
...
if [[ "${NOTARIZE:-0}" == "1" ]]; then
  ...
  echo "==> Rebuild DMG from stapled app"
  STAGE2="$(mktemp -d)"
  cp -R "${APP}" "${STAGE2}/"
  ln -s /Applications "${STAGE2}/Applications"
  hdiutil create -volname "Byze Core v${VER} arm64" -srcfolder "${STAGE2}" -ov -format UDZO "${DMG}"
  rm -rf "${STAGE2}"
  codesign --force --timestamp --sign "${IDENTITY}" "${DMG}"
  echo "==> Notarize DMG"
