#!/usr/bin/env bash
# Package Byze v0.2.4 macOS arm64 release (CLI + GUI + DMG)
set -euo pipefail

VER="0.2.4"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
BUILD="${ROOT}/build"
OUT="${ROOT}/release/v${VER}/macos-arm64"
...
"${ROOT}/release/v${VER}/resign-macos-app.sh" "${BUILD}/dist/Bitcoin-Qt.app" "-"
...
hdiutil create -volname "Byze Core v${VER} arm64" -srcfolder "${STAGE}" -ov -format UDZO "${DMG}"

echo "==> Checksums"
(
  cd "${ROOT}/release"
  shasum -a 256 "$(basename "${DMG}")" > SHA256SUMS_Mac_arm64.txt
  cd "v${VER}/macos-arm64"
...
echo "  ${ROOT}/release/v${VER}/sign-macos-release.sh"
